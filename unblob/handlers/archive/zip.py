...
def calculate_zipfile_end(file: io.BufferedIOBase, start_offset: int) -> int:
...
def calculate_chunk(
    ...
    end_of_zip = calculate_zipfile_end(file, start_offset)

    if end_of_zip == -1:
        return UnknownChunk(
            start_offset=start_offset,
            reason="Couldn't find the EOCD magic bytes.",
        )
